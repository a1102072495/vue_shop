...
export default {
    ...
    return {
      ...
      addFormRules: {
        username: [
          ...
          { min: 3, max: 10, message: '用户名在3~10个字符串之间', trigger: 'blur' }
        ],
        password: [
          { required: true, message: '请输入密码', trigger: 'blur' },
          { min: 6, max: 16, message: '密码在6~16个字符串之间', trigger: 'blur' }
        ],
        email: [
  ...
  methods: {
    ...
    // 监听 switch 开关状态的变化  改变用户状态
    async userStateChanged (userinfo) {
      // console.log(userinfo)
      const { data: res } = await this.$http.put(
        `users/${userinfo.id}/state/${userinfo.mgState}`
      )
      // console.log(res)
    ...
    async removeUserById (id) {
      ...
      // 如果用户选择确定，则返回字符串 'confirm'
      // 如果用户选择取消，则返回字符串 'cancel'
